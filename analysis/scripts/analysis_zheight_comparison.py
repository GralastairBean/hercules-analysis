...
def main() -> None:
    ...
    if plot_series.empty:
        raise ValueError("No classified stars with z-height values were found in the classified CSV.")

    z_limit = float(np.abs(plot_series).max())
    z_limit *= 1.03
    ...
    ax1.hist(other_df["z_height_pc"], bins=100, range=z_range, density=True, alpha=0.08, color="black")
    ...
    plt.suptitle("Vertical Structure Comparison: Hercules 1, Hercules 2, and Other", fontsize=14, y=0.98)
    plt.tight_layout()
